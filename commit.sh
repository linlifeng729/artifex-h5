...
# 远程仓库URL
REMOTE_REPO_URL="https://github.com/linlifeng729/artifex-h5.git"
...
# 第一步：检查是否有文件需要提交
echo "第一步：检查是否有文件需要提交..."
git add .

# 检查暂存区是否有内容
if git diff --cached --quiet; then
    echo "ℹ️  暂存区没有内容需要提交，跳过提交步骤"
    echo "直接进入推送步骤..."
else
    echo "✅ 已添加所有文件到暂存区"
    ...
    # 第二步：提示用户输入commit信息
    echo ""
    echo "第二步：请输入commit信息"
    echo "请输入commit信息（例如：feat: 添加新功能 或 fix: 修复bug）："
    read -r COMMIT_MESSAGE
    
    # 检查commit信息是否为空
    if [ -z "$COMMIT_MESSAGE" ]; then
        echo "❌ commit信息不能为空，请重新运行脚本"
        exit 1
    fi
...
if git push origin $BRANCH_NAME; then
    ...
    if [ ! -z "$GITLAB_USERNAME" ]; then
        REMOTE_REPO_URL_WITH_USER="https://${GITLAB_USERNAME}@github.com/linlifeng729/artifexH5.git"
...
# 推送步骤：切换并推送到目标仓库
echo "推送步骤：推送到目标仓库: $REMOTE_REPO_URL"
git remote set-url origin "$REMOTE_REPO_URL"
echo "已切换 origin 为: $REMOTE_REPO_URL"

echo "开始推送（尝试使用缓存的认证信息）..."
git push origin $BRANCH_NAME
# 完成步骤会在EXIT trap中自动执行
echo "🎉 一站式提交完成！已成功提交并推送到两个仓库"
